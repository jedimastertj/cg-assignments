...
int main(int, char**)
{
    ...
    // add a sphere
    Object *sphere = new Sphere(Vector3D(-2, 0, -10), 4, m);
    world->addObject(sphere);

    // add a triangle
    Triangle* triangle = new Triangle(Vector3D(0, 0, -5), Vector3D(4, 0, -5), Vector3D(2, 4, -5), m);
    world->addObject(triangle);

    // add a light source
    LightSource *light = new PointLightSource(world, Vector3D(4, 2, 0), Color(1, 1, 1));
