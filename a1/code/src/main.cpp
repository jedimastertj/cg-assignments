...
#define SAMPLES_PER_BEZIER 25 // Sample each Bezier curve as N = 10 segments and draw as connected lines

// Global variables
vector<float> controlPoints;
vector<float> linearBezier;
...
void calculatePiecewiseLinearBezier()
{
    ...
    float x[2], y[2];
    float delta_t = 1.0/(SAMPLES_PER_BEZIER - 1);
    float t;
...
void introduceCPs(pf& pt1, pf& pt2, pf& cp1, pf& cp2, pf& lastTangent, bool& constraint) {
    float deltaX = pt2.first - pt1.first, deltaY = pt2.second - pt1.second;
    if (deltaX == 0) deltaX = 0.5;
    if (deltaY == 0) deltaY = 0.5;

    if (constraint) {
        cp1.first = pt1.first + lastTangent.first;
        cp1.second = pt1.second + lastTangent.second;
    } else {
        cp1.first = pt1.first + 0.2 * deltaX;
        cp1.second = pt1.second + 0.6 * deltaY;
    }

    cp2.first = cp1.first + 0.6 * deltaX;
    cp2.second = cp1.second + 0.2 * deltaY;

    lastTangent.first = pt2.first - cp2.first;
    lastTangent.second = pt2.second - cp2.second;
}

void getPoint(pf& bezT, pf& pt1, pf& cp1, pf& cp2, pf& pt2, float t) {
    bezT.first = pt1.first * pow(1-t, 3) + 3 * cp1.first * t * pow(1-t, 2) + 3 * cp2.first * pow(t, 2) * (1-t) + pt2.first * pow(t, 3);
    bezT.second = pt1.second * pow(1-t, 3) + 3 * cp1.second * t * pow(1-t, 2) + 3 * cp2.second * pow(t, 2) * (1-t) + pt2.second * pow(t, 3);
}
...
void calculatePiecewiseCubicBezier()
{
    ...
    pf pt1, pt2, cp1, cp2, lastTangent, bezT;
    ...
    for(int i = 0; i < (sz-3); i += 3) {
        ...
        cubicBezier.push_back(0);

        pt1 = {x[0], y[0]};
        pt2 = {x[1], y[1]};
        introduceCPs(pt1, pt2, cp1, cp2, lastTangent, constraint);

        t = 0.0;
        for (float j = 1; j < (SAMPLES_PER_BEZIER - 1); j++)
        {
            t += delta_t;
            getPoint(bezT, pt1, cp1, cp2, pt2, t);
            cubicBezier.push_back(bezT.first);
            cubicBezier.push_back(bezT.second);
            cubicBezier.push_back(0);
        }
