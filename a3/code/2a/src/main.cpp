...
GLint lightPosition_uniform, lightDirection_uniform, lightCutoffAngle_uniform, lightColor_uniform, eyeNormal_uniform; // introduced uniforms
...
int main(int, char**)
{
    ...
    // Get handle to set value for light direction
    lightDirection_uniform = glGetUniformLocation(shaderProgram, "lightDirection");
    if(lightDirection_uniform == -1){
        fprintf(stderr, "Could not bind location: lightDirection \n");
        exit(0);
    }

    // Get handle to set value for light cutoff angle
    lightCutoffAngle_uniform = glGetUniformLocation(shaderProgram, "lightCutoffAngle");
    if(lightCutoffAngle_uniform == -1){
        fprintf(stderr, "Could not bind location: lightCutoffAngle \n");
        exit(0);
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform3f(lightPosition_uniform, 0, 0, -12);
        glUniform3f(lightDirection_uniform, 0, 0, 1);
        glUniform1f(lightCutoffAngle_uniform, 15 * RADIAN_PER_DEGREE);
